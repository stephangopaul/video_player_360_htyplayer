...
@property (strong, nonatomic) UIImage *playbackPauseImg;
@property (strong, nonatomic) UIImage *playbackPlayImg;
...
//- (id)initWithBundle:(NSBundle *)bundle url:(NSURL*)url {
...
- (void)updatePlayButton {
    [self.playButton setImage:[self isPlaying] ? self.playbackPauseImg : self.playbackPlayImg
...
#pragma mark - load images from bundle
- (void)configureImages {
    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"360_bundle" ofType:@"bundle"];
    NSBundle *bundle = [NSBundle bundleWithPath:bundlePath];

    self.playbackPlayImg = [UIImage imageNamed:@"play_land" inBundle:bundle compatibleWithTraitCollection:nil];
    self.playbackPauseImg = [UIImage imageNamed:@"pause_land" inBundle:bundle compatibleWithTraitCollection:nil];
    self.thumbImg = [UIImage imageNamed:@"thumb" inBundle:bundle compatibleWithTraitCollection:nil];
}
...
- (void)configureProgressSlider {
    ...
    [self.progressSlider setThumbImage:self.thumbImg forState:UIControlStateNormal];
    [self.progressSlider setThumbImage:self.thumbImg forState:UIControlStateHighlighted];
}
...
- (IBAction)gyroButtonTouched:(id)sender {
    if(self.glkViewController.isUsingMotion) {
        self.gyroButton.alpha = 0.5;
        [self.glkViewController stopDeviceMotion];
    } else {
        self.gyroButton.alpha = 1.0;
